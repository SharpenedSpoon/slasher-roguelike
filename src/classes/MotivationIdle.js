...
export default class Motivation {
	...
	_shiftAction() {
		if (this._actionsQueue.length > 0) {
			return this._actionsQueue.shift();
		}
	...
	_unshiftAction() {
		if (this._actionsQueue.length > 0) {
			return this._actionsQueue.unshift();
		}
	 ...
	 */
	_pushAction(action) {
		return this._actionsQueue.push(action);
	}
}
